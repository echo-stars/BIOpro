...
y_train = np.array(y_train,dtype='float16') # transformed as np.array for GNN model
...
y_test = np.array(y_test,dtype='float16') # transformed as np.array for GNN model
...
def ESM_GCN(X_train, y_train, X_test, y_test):


  inputShape = (320,1)
  input = Input(inputShape)
  x = Conv1D(128, 3, padding='same')(input)
...
for i in range(20):
    ...
    model, model_history = ESM_GCN(X_train, y_train, X_test , y_test)
    # confusion matrix
    predicted_class= []
...
for train_index , test_index in kf.split(y_train):
    ...
    model, model_history = ESM_GCN(X_train_CV, y_train_CV, X_valid_CV, y_valid_CV)
    # confusion matrix
    predicted_class= []
...
# loading the y dataset for model development
y_train = dataset['Label']
y_train = np.array(y_train) # transformed as np.array for GNN model
# test dataset loading
dataset = pd.read_excel('../data/orginal/all_test.xlsx',na_filter = False) # take care the NA sequence problem
sequence_list = dataset['SequenceID']
# loading the y dataset for model development
y_test = dataset['Label']
y_test = np.array(y_test) # transformed as np.array for GNN model
# assign the dataset
X_train_data_name = 'new_neuro_train_esm2_t6_8M_UR50D_unified_320_dimension.csv'
X_train_data = pd.read_csv(X_train_data_name,header=0, index_col = 0,delimiter=',')
